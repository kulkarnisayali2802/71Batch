...
- generic [ref=e1]:
  ...
  - iframe [active] [ref=e311]:
    - generic [ref=f1e1]:
      - generic [ref=f1e4]:
        - generic [ref=f1e5]: Loading, please wait...
        - generic [ref=f1e6]:
          - img
      - iframe [ref=f1e8]:
        
      - button "Back" [ref=f1e10]
      - generic [ref=f1e12]:
        - generic [ref=f1e18]:
          - 'heading "PIM : Add Employee" [level=2] [ref=f1e20]'
          - generic [ref=f1e21]: Code
          - textbox "Code" [ref=f1e22]: "8884"
          - generic [ref=f1e23]: Last Name *
          - textbox "Last Name *" [active] [ref=f1e24]: Ameerpet
          - generic [ref=f1e25]: First Name *
          - textbox [ref=f1e26]
          - generic [ref=f1e27]: Middle Name
          - textbox "Middle Name" [ref=f1e28]
          - generic [ref=f1e29]: Nick Name
          - textbox "Nick Name" [ref=f1e30]
          - generic [ref=f1e31]: Photo
          - button "Photo" [ref=f1e32]
          - generic [ref=f1e33]:
            - button "Save" [ref=f1e34]
            - button "Reset" [ref=f1e35]
        - generic [ref=f1e40]: Fields marked with an asterisk * are required.
...
```ts
  1  | //Toprovide all Reusable Functions? Methods[User-Defined] related to whole application
  2  | 
  ...
  14 | 
  15 |     async login(){
  16 |     await this.page.locator(this.textbox_loginname).fill(this.username);
  17 |     await this.page.locator(this.textbox_password).fill(this.password)
  18 |     console.log("username and password entered");
  ...
  23 | 
  24 |     async logout(){
  25 |         //await this.page.locator(this.link_logout).click();
  26 |         await this.page.getByRole('link',{name:this.link_logout})
  27 |         console.log("Logout successful");
  28 |     }
  29 | 
  30 |     async waitStatement(){
  31 |         await this.page.waitForTimeout(3000);
  32 |     }
  33 | 
  34 |     async addEmployee(){
  35 |         const frame= await this.page.frameLocator(this.frame_empInfo);
  36 |         await frame.locator(this.add_button).click();
  37 | 
  38 |         console.log("clicked on add button");
  39 |         await frame.locator(this.textfield_lastname).fill(this.lastname);
> 40 |         await frame.locator(this.textfield_Firstname).fill(this.firstname);
     |                                                       ^ Error: locator.fill: Error: strict mode violation: locator('#txtEmpFirstName') resolved to 2 elements:
  41 |         //await frame.locator(this.textfield_Firstname).fill(this.firstname);
  42 |         await frame.locator(this.textfiled_Middlename).fill(this.middlename);
  43 |         await frame.locator(this.textfield_Nickname).fill(this.nickname);
  44 | 
  45 |         await frame.locator(this.button_Save).click();
  46 |         console.log("clicked on save button, employee successfully created");
  47 | }
  48 | }
```